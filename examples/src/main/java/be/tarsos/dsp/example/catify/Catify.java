...
public class Catify {
        ...
        try {
            if (args.length == 0) {
                ...
                if (dir.isDirectory()) {
                    ...
                    throw new IllegalArgumentException("Third argument should be a directory containing wav files.");
                }
            }
        } catch (Exception e) {
            printDescription();
    ...
    public static void copyFileFromJar(final String source, final String target) {
        try (InputStream inputStream = MidiNoteInfo.class.getResourceAsStream(source)) {

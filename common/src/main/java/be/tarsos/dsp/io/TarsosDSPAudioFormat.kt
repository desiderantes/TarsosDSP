 ...
 */(
    ...
    constructor(
        ...
        if (signed) Encoding.PCM_SIGNED else Encoding.PCM_UNSIGNED,
        sampleRate,
        sampleSizeInBits,
        channels,
     ...
     */
    fun matches(format: TarsosDSPAudioFormat): Boolean {
    ...
    override fun toString(): String {
        ...
        return listOf(
            "$encoding ",
            if (sampleRate == NOT_SPECIFIED.toFloat()) {
                "unknown sample rate"
            } else {
                "$sampleRate Hz"
            },
            if (sampleSizeInBits.toFloat() == NOT_SPECIFIED.toFloat()) {
            ...
            if (frameSize.toFloat() == NOT_SPECIFIED.toFloat()) {
                "unknown frame size"
            } else {
                "$frameSize bytes/frame"
            },
            if (abs(sampleRate - frameRate) > 0.00001) {

...
import be.tarsos.dsp.util.AudioResourceUtils.sanitizeResource
 ...
 */
class PipedAudioStream(resource: String) {
    private val resource: String = sanitizeResource(resource)
    private val decoder: PipeDecoder
     ...
     */
    fun getMonoStream(targetSampleRate: Int, startTimeOffset: Double): TarsosDSPAudioInputStream {
        return getMonoStream(targetSampleRate, startTimeOffset, -1.0)
    }

    private fun getTargetFormat(targetSampleRate: Int): TarsosDSPAudioFormat {
        return TarsosDSPAudioFormat(targetSampleRate.toFloat(), 16, 1, true, false)
    }
    ...
    companion object {
        ...
        private var pipeDecoder = PipeDecoder()
        fun setDecoder(decoder: PipeDecoder) {
            pipeDecoder = decoder
        }
    }

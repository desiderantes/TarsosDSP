 ...
 * article [A Smarter Way to Find Pitch](http://miracle.otago.ac.nz/tartini/papers/A_Smarter_Way_to_Find_Pitch.pdf). According to the article:
 *
 ...
 * to `O((W+w)log(W+w))` by using an <abbr title="Fast Fourier Transform">FFT</abbr> to calculate the <abbr title="Auto-Correlation Function">ACF</abbr>. But I am still afraid of the
 ...
 * @author Joren Six
 */
class McLeodPitchMethod @JvmOverloads constructor(
    /**
     ...
     */
    private val cutoff: Double = cutoffMPM

    /**
     * Contains a normalized square difference function value for each delay
     * (tau).
     */
    private val nsdf: FloatArray = FloatArray(audioBufferSize)

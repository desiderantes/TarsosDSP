/*
 *      _______                       _____   _____ _____
 *     |__   __|                     |  __ \ / ____|  __ \
 *        | | __ _ _ __ ___  ___  ___| |  | | (___ | |__) |
 *        | |/ _` | '__/ __|/ _ \/ __| |  | |\___ \|  ___/
 *        | | (_| | |  \__ \ (_) \__ \ |__| |____) | |
 *        |_|\__,_|_|  |___/\___/|___/_____/|_____/|_|
 *
 * -------------------------------------------------------------
 *
 * TarsosDSP is developed by Joren Six at IPEM, University Ghent
 *
 * -------------------------------------------------------------
 *
 *  Info: http://0110.be/tag/TarsosDSP
 *  Github: https://github.com/JorenSix/TarsosDSP
 *  Releases: http://0110.be/releases/TarsosDSP/
 *
 *  TarsosDSP includes modified source code by various authors,
 *  for credits and info, see README.
 *
 */
package be.tarsos.dsp.wavelet

import kotlin.math.sqrt

class HaarWaveletTransform @JvmOverloads constructor(private val preserveEnergy: Boolean = false) {
    private val sqrtTwo = sqrt(2.0).toFloat()

    /**
     * Does an in-place HaarWavelet wavelet transform. The
     * length of data needs to be a power of two.
     * It is based on the algorithm found in "Wavelets Made Easy" by Yves Nivergelt, page 24.
     *
     * @param s The data to transform.
     */
    fun transform(s: FloatArray) {
        var m = s.size
        assert(isPowerOfTwo(m))
        val n = log2(m)
        var j = 2
        var i = 1
        for (l in 0 until n) {
            m /= 2
            for (k in 0 until m) {
                var a = (s[j * k] + s[j * k + i]) / 2.0f
                var c = (s[j * k] - s[j * k + i]) / 2.0f
                if (preserveEnergy) {
                    a /= sqrtTwo
                    c /= sqrtTwo
                }
                s[j * k] = a
                s[j * k + i] = c
            }
            i = j
            j *= 2
        }
    }

    /**
     * Does an in-place inverse HaarWavelet Wavelet Transform. The data needs to be a power of two.
     * It is based on the algorithm found in "Wavelets Made Easy" by Yves Nivergelt, page 29.
     *
     * @param data The data to transform.
     */
    fun inverseTransform(data: FloatArray) {
        var m = data.size
        assert(isPowerOfTwo(m))
        val n = log2(m)
        var i = pow2(n - 1)
        var j = 2 * i
        m = 1
        for (l in n downTo 1) {
            for (k in 0 until m) {
                var a = data[j * k] + data[j * k + i]
                var a1 = data[j * k] - data[j * k + i]
                if (preserveEnergy) {
                    a *= sqrtTwo
                    a1 *= sqrtTwo
                }
                data[j * k] = a
                data[j * k + i] = a1
            }
            j = i
            i /= 2
            m *= 2
        }
    }

    companion object {
        /**
         * Checks if the number is a power of two. For performance it uses bit shift
         * operators. e.g. 4 in binary format is
         * "0000 0000 0000 0000 0000 0000 0000 0100"; and -4 is
         * "1111 1111 1111 1111 1111 1111 1111 1100"; and 4&-4 will be
         * "0000 0000 0000 0000 0000 0000 0000 0100";
         *
         * @param number The number to check.
         * @return True if the number is a power of two, false otherwise.
         */
        fun isPowerOfTwo(number: Int): Boolean {
            require(number > 0) { "number: $number" }
            return number and -number == number
        }

        /**
         * A quick and simple way to calculate log2 of integers.
         *
         * @param bits the integer
         * @return log2(bits)
         */
        fun log2(bits: Int): Int {
            return if (bits == 0) {
                0
            } else 31 - Integer.numberOfLeadingZeros(bits)
        }

        /**
         * A quick way to calculate the power of two (2^power), by using bit shifts.
         *
         * @param power The power.
         * @return 2^power
         */
        fun pow2(power: Int): Int {
            return 1 shl power
        }
    }

}
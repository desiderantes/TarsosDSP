 ...
 * @author Ian Kaplan
 */
class PolynomialWavelets : LiftingSchemeBaseWavelet() {
    ...
    override fun update(
        ...
        for (i in 0 until half) {
            ...
            vec[i] = when (direction) {
                TransformDirection.FORWARD -> (vec[i] + vec[j]) / 2
                TransformDirection.INVERSE -> 2 * vec[i] - vec[j]
            }
        }
    }
 ...
 * http://www.bearcave.com/misl/misl_tech/wavelets/lifting/index.html](http://www.bearcave.com/misl/misl_tech/wavelets/lifting/index.html)
    ...
    override fun predict(
        ...
        val half = N shr 1
        val d = FloatArray(numPts)

        // int k = 42;
        for (i in 0 until half) {
            var predictVal: Float = if (i == 0) {
                if (half == 1) {
            ...
            val j = i + half
            vec[j] = when (direction) {
                TransformDirection.FORWARD -> vec[j] - predictVal
                TransformDirection.INVERSE -> vec[j] + predictVal
            }
        }
    }

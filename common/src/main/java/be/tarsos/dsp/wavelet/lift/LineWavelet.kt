 ...
 * @author Ian Kaplan
 */
class LineWavelet : LiftingSchemeBaseWavelet() {
    /**
     *
     *
     ...
     */
    private fun new_y(y1: Float, y2: Float): Float {
        return 2 * y2 - y1
    }
     ...
     *
     */
    override fun predict(vec: FloatArray, N: Int, direction: TransformDirection) {
        val half = N shr 1
        var predictVal: Float
        for (i in 0 until half) {
            val j = i + half
            predictVal = if (i < half - 1) {
                (vec[i] + vec[i + 1]) / 2
            } else if (N == 2) {
                vec[0]
            } else {
     ...
     */
    override fun update(vec: FloatArray, N: Int, direction: TransformDirection) {
        val half = N shr 1
        for (i in 0 until half) {
            val j = i + half
            var `val`: Float

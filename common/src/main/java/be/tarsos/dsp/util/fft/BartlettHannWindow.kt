 ...
 * @author Corban Brook
 * @see [The Bartlett-Hann Window](http://en.wikipedia.org/wiki/Window_function.Bartlett.E2.80.93Hann_window)
 */
 ...
 */
    ...
    override fun value(length: Int, index: Int): Float {
        return (0.62 - 0.48 * abs(index / (length - 1) - 0.5) - 0.38 * cos(
            TWO_PI * index / (length - 1).toDouble()
        )).toFloat()
    }
}